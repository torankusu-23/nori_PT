...
//存储采样结果的结构体，要求均匀采样每一个triangle，并且存储其位置，法线，和pdf
...
public:
    ...
    //返回均匀采样cdf
    const DiscretePDF& getpdf() const {return mesh_cdf;}
    //采样函数
...
protected:
    ...
    DiscretePDF mesh_cdf;//均匀采样的一个离散结构，主要是一个存储cdf的vector，可以用来invertion采样
    float       mesh_area;//网格总表面积，pdf = 面积分之一
