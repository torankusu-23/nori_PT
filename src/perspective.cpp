...
public:
    ...
    Color3f sampleRay(Ray3f &ray,
        ...
        Point3f nearP = m_sampleToCamera * Point3f(//视口平面采样坐标，outputSize默认1280*720
            samplePosition.x() * m_invOutputSize.x(),
            samplePosition.y() * m_invOutputSize.y(), 0.0f);

        /* Turn into a normalized ray direction, and
           adjust the ray interval accordingly */
        Vector3f d = nearP.normalized();//归一化得到单位光线方向
        float invZ = 1.0f / d.z();

        ray.o = m_cameraToWorld * Point3f(0, 0, 0);//o是光源坐标
        ray.d = m_cameraToWorld * d;//d是光线方向，光线函数：o + t * d
        ray.mint = m_nearClip * invZ;//最小t
        ray.maxt = m_farClip * invZ;//最大t
        ray.update();
